# filename: blackhole-enhanced-final.py
# ENHANCED VERSION WITH:
# 1. Smart documentation-aware code generation (checks latest official docs)
# 2. Robust fallback system (automatically falls back to Gemini if other models fail)
...
python blackhole-enhanced-final.py

For self-tests:
python blackhole-enhanced-final.py --selftest
...
class MultiAI:
    ...
    def try_fallback_to_gemini(self, context: str, original_error: str) -> Tuple[str, Dict[str, Any]]:
        """Fallback to Gemini if other models fail"""
        print("🔄 Attempting fallback to Gemini...")
        
        if not self.google_models:
            return f"All models failed. Original error: {original_error}", {
        ...
        try:
            ...
            fallback_info = {
                ...
                "model_name": "Google Gemini (Fallback)",
                "model_icon": "🌟",
                "fallback_used": True,
                "original_error": original_error,
                "reasoning": "Fallback to Gemini after primary model failed"
            }
            
            print("✅ Fallback to Gemini successful")
            return response_text, fallback_info
            
        except Exception as fallback_error:
            print(f"❌ Fallback to Gemini also failed: {fallback_error}")
            return f"All models failed. Original: {original_error}. Fallback: {fallback_error}", {
    ...
    def generate_context_aware_response(self, prompt: str, chat_id: str, use_routing: bool = True, manual_model: str = None) -> Tuple[str, Dict[str, Any]]:
        ...
        if not model:
            print(f"⚠️ Primary model '{routing_info.get('selected_model')}' not available, trying fallback")
            return self.try_fallback_to_gemini(context, "Primary model not available")
...
These are basic "Hello World" programs to get you started with programming!"""
                    ...
                    # Fallback to Gemini even if we're already using Google (different model)
                    return self.try_fallback_to_gemini(context, f"Google model error: {e}")
                ...
                try:
                    ...
                    return self.try_fallback_to_gemini(context, f"OpenAI model error: {e}")
                ...
                try:
                    ...
                    return self.try_fallback_to_gemini(context, f"Claude model error: {e}")
            ...
            return self.try_fallback_to_gemini(context, f"Unexpected error: {err}")
...
def main(page: ft.Page):
    page.title = "🕳️ BLACKHOLE AI - Enhanced"
    ...
    routing_switch = ft.Row([
        ...
        ft.Text("Smart Routing", color=ft.colors.PURPLE_400, size=14)
    ], spacing=8)
    ...
    def create_message_bubble(message: Message) -> ft.Container:
        if message.role == "user":
            ...
            if message.routing_info and message.routing_info.get("fallback_used"):
                model_name += " (Fallback)"
                model_icon = "🔄"
            ...
            if message.routing_info and message.routing_info.get("fallback_used"):
                fallback_indicator = ft.Container(
                    content=ft.Row([
                        ft.Icon(ft.icons.AUTORENEW, size=14, color=ft.colors.ORANGE_300),
                        ft.Text("Used fallback after primary model failed", size=11, color=ft.colors.ORANGE_300)
                    ], spacing=8),
                ...
                controls.append(fallback_indicator)
        ...
        if not current_chat_id:
            welcome = ft.Container(
                content=ft.Column([
                    ...
                    ft.Text("Enhanced AI Assistant", size=16, weight=ft.FontWeight.W_500,
                    ...
                    ft.Text("🔹 Automatic fallback to Gemini if other models fail", 
            ...
            try:
                ...
                def update_ui():
                    ...
                    if routing_info.get("fallback_used"):
                        status_msg += " (fallback used)"
                        status_color = ft.colors.ORANGE_400
                    elif routing_info.get("doc_search_triggered"):
        ...
        content=ft.Column([
            ft.Container(
                content=ft.Column([
                    ...
                    ft.Text("AI Enhanced", size=13, color=ft.colors.BLUE_400, weight=ft.FontWeight.W_500)
    ...
    header = ft.Container(
        content=ft.Row([
            ft.Column([
                current_chat_title,
                ft.Row([
                    ft.Text("📚 Doc-Aware", size=12, color=ft.colors.PURPLE_400, weight=ft.FontWeight.W_500),
                    ft.Text("•", size=12, color=ft.colors.GREY_500),
                    ft.Text("🔄 Auto-Fallback", size=12, color=ft.colors.ORANGE_400, weight=ft.FontWeight.W_500),
                    ft.Text("•", size=12, color=ft.colors.GREY_500),
                    ft.Text(f"Today: {CURRENT_DATE}", size=12, color=ft.colors.GREEN_400)
                ], spacing=8)
            ], expand=True),
            ft.Row([
                model_dropdown,
    ...
    input_field.focus()
    update_status("Enhanced AI ready - Smart docs & fallback enabled", ft.colors.GREEN_400)
    print("✅ BLACKHOLE AI Enhanced ready!")

def run_self_tests():
    print("🧪 Running enhanced self-tests...\n")
    ...
    print("✅ Enhanced self-tests completed!")
